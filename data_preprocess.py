...
audio_path = r'/lc_work/wanghong/Speaker_Verification_vox/Data_vox_1/wav'         # utterance dataset
clean_path = r'/lc_work/wanghong/Speaker_Verification/data/files/clean_testset_wav'  # clean dataset
noisy_path = r'/lc_work/wanghong/Speaker_Verification/data/files/noisy_testset_wav'  # noisy dataset
...
def save_spectrogram_tisv():
    ...
    for id_index, id_folder in enumerate(os.listdir(audio_path)):



        id_folder_path = os.path.join(audio_path, id_folder)
        print("%s,  %dth speaker processing..."%(id_folder, id_index)) #id_folder[-5:] 取出文件名也就是id数字
        
        utterances_spec = []
        k=0

        # print(type(int(id_folder[-5:])))
        # quit()

        for i, audio_file in enumerate(os.listdir(id_folder_path)):

            speaker_path = os.path.join(id_folder_path, audio_file)     # path of each speaker
            
            for utter_name in os.listdir(speaker_path):
                utter_path = os.path.join(speaker_path, utter_name)         # path of each utterance
                # print(i, folder,utter_path, utter_name)                   # printing for test
        ...
        if int(id_folder[-4:])<train_speaker_num:      # save spectrogram as numpy file
            np.save(os.path.join(config.train_path, "speaker%d.npy"%int(id_folder[-4:])), utterances_spec)
        else:
            np.save(os.path.join(config.test_path , "speaker%d.npy"%(int(id_folder[-4:]-train_speaker_num))), utterances_spec)
